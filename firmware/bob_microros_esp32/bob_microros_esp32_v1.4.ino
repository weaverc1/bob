 ...
 *
 * Version History:
 * v1.4 (2025-10-22): **BOOT CYCLING FIX**
 * - FIXED: Increased watchdog timeout from 3→5 seconds to prevent premature resets
 * - FIXED: Reduced timer frequency from 100Hz→50Hz to reduce micro-ROS serial stress
 * - FIXED: Added I2C timeout (200ms) for IMU reads to prevent blocking
 * - FIXED: Removed race condition in encoder ISRs (don't modify last_encoder_*)
 * - ADDED: Boot cycle detection and diagnostic logging
 * - ADDED: IMU failure graceful degradation (continues without IMU)
 * - ADDED: Watchdog reset in loop() for additional safety
...
#define WATCHDOG_TIMEOUT_SEC  5     // Watchdog timeout in seconds (INCREASED from 3)
#define MAX_LINEAR_VEL        0.5   // Maximum linear velocity (m/s)
#define MAX_ANGULAR_VEL       2.0   // Maximum angular velocity (rad/s)
#define IMU_TIMEOUT_MS        200   // I2C timeout for IMU reads (NEW)
...
#define MOTOR_RIGHT_DIR1  32    // Right motor direction pin 1
#define MOTOR_RIGHT_DIR2  14    // Right motor direction pin 2

// Encoders (NOTE: GPIO 34 and 35 are INPUT_ONLY on ESP32)
...
bool imu_available = false;  // NEW: Track IMU availability
unsigned long last_imu_read = 0;  // NEW: For I2C timeout detection

// ==================== SYSTEM STATE ====================
bool watchdog_initialized = false;  // Prevent double initialization
unsigned long boot_count = 0;  // NEW: Boot cycle detection
unsigned long last_boot_time = 0;  // NEW: Boot cycle detection
...
void error_loop() {
  Serial.println("[ERROR] Entering error loop - check micro-ROS connection");
  while(1) {
...
// FIXED v1.4: Removed race condition - ISRs no longer modify last_encoder_* variables
...
void setup_encoders() {
  ...
  Serial.println("[INIT] Encoders configured on GPIO 34 (left) and 35 (right)");
}

// ==================== IMU SETUP ====================
// IMPROVED v1.4: Better error handling and graceful degradation
bool setup_imu() {
  Serial.println("[INIT] Initializing BNO086 IMU...");
  ...
  if (!bno08x.enableReport(SH2_ARVR_STABILIZED_RV, 20000)) {  // CHANGED: 50Hz = 20000 us (was 100Hz)
    Serial.println("[WARN] Failed to enable IMU rotation vector");
    imu_available = false;
    return false;
  }
  if (!bno08x.enableReport(SH2_ACCELEROMETER, 20000)) {  // CHANGED: 50Hz = 20000 us
    Serial.println("[WARN] Failed to enable IMU accelerometer");
    imu_available = false;
    return false;
  }
  if (!bno08x.enableReport(SH2_GYROSCOPE_CALIBRATED, 20000)) {  // CHANGED: 50Hz = 20000 us
    Serial.println("[WARN] Failed to enable IMU gyroscope");
    imu_available = false;
    return false;
  }

  Serial.println("[INIT] IMU initialized successfully at 50Hz");
  imu_available = true;
  return true;
}
...
void compute_odometry() {
  ...
  // Get encoder deltas (FIXED v1.4: Safe snapshot, no race condition)
...
// ==================== IMU READING ====================
// IMPROVED v1.4: Added timeout protection to prevent I2C blocking
void read_and_publish_imu() {
  if (!imu_available) {
    return;  // Skip if IMU not available
  }

  if (bno08x.wasReset()) {
    Serial.println("[WARN] IMU reset detected, reinitializing...");
    setup_imu();
    return;
  }

  // NEW v1.4: I2C timeout protection
  unsigned long imu_start = millis();

  if (!bno08x.getSensorEvent(&sensorValue)) {
    // Check for timeout
    if (millis() - imu_start > IMU_TIMEOUT_MS) {
      Serial.println("[WARN] IMU read timeout");
    }
    return;
  }

  last_imu_read = millis();
  unsigned long current_time = last_imu_read;
...
// ==================== TIMER CALLBACK ====================
// CHANGED v1.4: Timer runs at 50Hz instead of 100Hz to reduce load
...
void setup() {
  ...
  // NEW v1.4: Boot cycle detection
  boot_count++;
  unsigned long current_time = millis();
  if (current_time - last_boot_time < 10000) {  // Less than 10 seconds since last boot
    Serial.printf("[WARN] Rapid boot detected! Boot #%lu in 10 seconds\n", boot_count);
  }
  last_boot_time = current_time;

  Serial.println("\n\n========================================");
  Serial.println("BOB ESP32 Micro-ROS Firmware v1.4");
  Serial.println("========================================");
  ...
  // CHANGED v1.4: Increased timeout from 3 to 5 seconds
  if (!watchdog_initialized) {
    Serial.printf("[INIT] Initializing watchdog timer (%d seconds)...\n", WATCHDOG_TIMEOUT_SEC);
    #if ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(3, 0, 0)
      ...
      if (err == ESP_OK) {
        ...
        Serial.println("[INIT] Watchdog timer initialized");
      } else {
        Serial.println("[WARN] Watchdog already initialized (OK)");
      }
    #else
      // ESP32 Arduino 2.x API
  ...
  Serial.println("[INIT] Setting up encoders...");
  setup_encoders();

  // IMU is optional - graceful degradation if it fails
  if (!setup_imu()) {
    Serial.println("[WARN] Continuing without IMU - odometry from encoders only");
  }

  // Initialize micro-ROS transport
  Serial.println("[INIT] Initializing micro-ROS transport...");
  ...
  // Create init_options
  Serial.println("[INIT] Initializing micro-ROS support...");
  RCCHECK(rclc_support_init(&support, 0, NULL, &allocator));

  // Create node
  Serial.println("[INIT] Creating ROS2 node 'bob_esp32_node'...");
  RCCHECK(rclc_node_init_default(&node, "bob_esp32_node", "", &support));

  // Create publishers
  Serial.println("[INIT] Creating /odom publisher...");
  ...
  Serial.println("[INIT] Creating /imu publisher...");
  RCCHECK(rclc_publisher_init_default(
    &imu_publisher,
    &node,
  ...
  // Create subscriber
  Serial.println("[INIT] Creating /cmd_vel subscriber...");
  ...
  // Create timer (CHANGED v1.4: 50Hz = 20ms, was 100Hz = 10ms)
  const unsigned int timer_timeout = 20;  // 50Hz
  Serial.printf("[INIT] Creating timer at 50Hz (%dms)...\n", timer_timeout);
  ...
  // Create executor
  Serial.println("[INIT] Creating executor...");
  ...
  Serial.println("========================================");
  Serial.println("Subscribed: /cmd_vel");
  Serial.println("Publishing: /odom, /imu");
  Serial.printf("IMU Status: %s\n", imu_available ? "ACTIVE" : "DISABLED");
  Serial.println("========================================\n");
...
// ==================== MAIN LOOP ====================
// IMPROVED v1.4: Added watchdog reset here as well for extra safety
void loop() {
  // Reset watchdog in loop as well (in addition to timer callback)
  esp_task_wdt_reset();
